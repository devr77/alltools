...
// Header and footer for Share, used by app/share/layout.tsx (toolsbase.org/share) and app/share-domain/layout.tsx.
...
export default function ShareShell({ place, children }: { place: Place; children: React.ReactNode }) {
  return (
      ...
      <header className="topbar">
        <div className="inner">
          ...
          <nav aria-label="Share tools">
            ...
            <Link href={`${place.home}#all-tools`}>All tools</Link>
          </nav>
        </div>
      </header>
          ...
          <div className="inner">
            ...
            <div>
              ...
              <p>
                ...
                and they are deleted automatically when they expire. To report a link, <Link href={`${place.main}/contact`}>contact us</Link> with the URL.
              </p>
            </div>
          </div>
        </aside>
        ...
        <div className="inner">
          <div className="footer-grid">
            ...
            <nav aria-label="Media tools">
              <h2>Media</h2>
              {media.map((tool) => <Link key={tool.slug} href={`${place.path}/${tool.slug}`}>{tool.name}</Link>)}
            </nav>
            <nav aria-label="Document and data tools">
              <h2>Documents &amp; data</h2>
              {docs.map((tool) => <Link key={tool.slug} href={`${place.path}/${tool.slug}`}>{tool.name}</Link>)}
            </nav>
            <nav aria-label="Help and policies">
              <h2>Help</h2>
              <Link href={`${place.main}/about`}>About</Link>
              <Link href={`${place.main}/privacy`}>Privacy policy</Link>
              <Link href={`${place.main}/contact`}>Contact &amp; report abuse</Link>
            </nav>
          </div>
