...
export function bencode(value) {
    if (typeof value === "number") {
        if (!Number.isSafeInteger(value))
            throw new Error("Bencode integers must be safe whole numbers.");
    ...
    if (Array.isArray(value))
        return join([encoder.encode("l"), ...value.map(bencode), encoder.encode("e")]);
...
function decodeTorrent(bytes) {
    if (!bytes.length || bytes.length > MAX_TORRENT_BYTES)
        throw new Error("Choose a non-empty .torrent file up to 10 MiB.");
    let offset = 0;
    let nodes = 0;
    let infoBytes;
    function parse(depth = 0) {
        if (depth > 64 || ++nodes > 200000)
            throw new Error("Torrent metadata is too complex.");
        const tag = bytes[offset++];
        if (tag === 105) {
            const end = bytes.indexOf(101, offset);
            if (end < 0 || end - offset > 17)
            ...
            if (!/^(0|-?[1-9]\d*)$/.test(text) || !Number.isSafeInteger(value))
                throw new Error("Invalid or unsupported bencode integer.");
            offset = end + 1;
            return value;
        }
        if (tag === 108 || tag === 100) {
            ...
            while (offset < bytes.length && bytes[offset] !== 101) {
                ...
                if (!(keyBytes instanceof Uint8Array))
                    throw new Error("Dictionary keys must be byte strings.");
                if (previousKey && compareBytes(previousKey, keyBytes) >= 0)
                    throw new Error("Torrent dictionary keys must be sorted and unique.");
                previousKey = keyBytes;
                const key = decoder.decode(keyBytes);
                const start = offset;
                dict[key] = parse(depth + 1);
                if (depth === 0 && key === "info")
                    infoBytes = bytes.subarray(start, offset);
            }
            if (bytes[offset++] !== 101)
                throw new Error("Truncated bencode container.");
            return tag === 108 ? list : dict;
        }
        if (tag >= 48 && tag <= 57) {
            const start = offset - 1;
            const colon = bytes.indexOf(58, start);
            if (colon < 0 || colon - start > 10)
                throw new Error("Invalid byte string length.");
            ...
            if (offset > bytes.length)
                throw new Error("Truncated byte string.");
    ...
    if (offset !== bytes.length)
        throw new Error("Unexpected data after torrent metadata.");
    if (!infoBytes)
...
function safeName(name) {
    if (!name.trim() || name === "." || name === ".." || /[\\/\x00-\x1f\x7f]/.test(name))
        throw new Error("File and folder names must be non-empty and cannot contain slashes or control characters.");
    return name;
}
async function sha1(bytes) {
    if (!globalThis.crypto?.subtle)
        throw new Error("Hashing requires a browser with Web Crypto on HTTPS or localhost.");
...
export function normalizeInfoHash(input) {
...
export function parseTrackers(input) {
    const trackers = [...new Set(input.split(/\r?\n/).map((line) => line.trim()).filter(Boolean))];
    if (trackers.length > 50)
        throw new Error("Use up to 50 tracker URLs.");
...
export function makeMagnet(hash, name = "", trackers = []) {
...
export function extractMagnet(input) {
    let url;
    try {
        url = new URL(input.trim());
    }
    catch {
        throw new Error("Enter a valid magnet link beginning with magnet:?.");
    }
    if (url.protocol !== "magnet:")
        throw new Error("Enter a magnet link beginning with magnet:?.");
    const hashes = url.searchParams.getAll("xt").filter((value) => /^urn:btih:/i.test(value)).map(normalizeInfoHash);
    if (!hashes.length)
        throw new Error("No BitTorrent v1 (urn:btih) hash found. V2-only magnet links are not supported.");
    if (new Set(hashes).size !== 1)
        throw new Error("This magnet contains conflicting BitTorrent v1 hashes.");
...
export async function inspectTorrent(bytes) {
    const { root, infoBytes } = decodeTorrent(bytes);
    const info = dictionary(root.info, "Info");
    if (!(info.pieces instanceof Uint8Array))
        throw new Error("This file has no v1 piece hashes. V2-only torrents are not supported.");
    if (info.pieces.length % 20)
        throw new Error("Piece hashes must be a multiple of 20 bytes.");
    const pieceLength = nonnegative(info["piece length"], "Piece length");
    if (pieceLength === 0)
        throw new Error("Piece length must be greater than zero.");
    const name = safeName(text(info.name));
    if ((info.length !== undefined) === (info.files !== undefined))
        throw new Error("Torrent must contain either a length or a file list.");
    const files = [];
    if (info.length !== undefined)
        files.push({ path: name, length: nonnegative(info.length, "File length") });
    else {
        if (!Array.isArray(info.files) || !info.files.length)
            throw new Error("Torrent file list is empty or invalid.");
        for (const entry of info.files) {
            const file = dictionary(entry, "File");
            if (!Array.isArray(file.path) || !file.path.length)
    ...
    if (new Set(files.map((file) => file.path)).size !== files.length)
        throw new Error("Torrent contains duplicate file paths.");
    const totalSize = files.reduce((sum, file) => sum + file.length, 0);
    if (!Number.isSafeInteger(totalSize))
        throw new Error("Torrent total size exceeds the supported integer range.");
    const pieceCount = info.pieces.length / 20;
    if (pieceCount !== Math.ceil(totalSize / pieceLength))
        throw new Error("Piece hash count does not match the declared content size.");
...
export async function createTorrent(files, options = {}) {
    if (!files.length || files.length > MAX_FILES)
        throw new Error("Choose between 1 and 1,000 files.");
    const totalSize = files.reduce((sum, file) => sum + file.size, 0);
    if (totalSize > MAX_CONTENT_BYTES)
        throw new Error("Select up to 1 GiB of content per torrent.");
    const pieceLength = options.pieceLength ?? 256 * 1024;
    if (!Number.isInteger(pieceLength) || pieceLength < 16384 || pieceLength > 16 * 1024 * 1024 || (pieceLength & (pieceLength - 1)) !== 0)
        throw new Error("Choose a power-of-two piece size between 16 KiB and 16 MiB.");
    const trackers = parseTrackers((options.trackers || []).join("\n"));
    if (options.private && !trackers.length)
        throw new Error("A private torrent needs at least one tracker URL.");
    const names = files.map((file) => safeName(file.name));
    if (new Set(names).size !== names.length)
        throw new Error("Selected files have duplicate names. Rename them before creating a torrent.");
    ...
    const checkCanceled = () => { if (options.signal?.aborted)
        throw new Error("Torrent creation canceled."); };
    ...
    checkCanceled();
    if (used)
        hashes.push(await sha1(piece.subarray(0, used)));
    const info = { name, "piece length": pieceLength, pieces: join(hashes) };
    if (files.length === 1)
        info.length = totalSize;
    else
        info.files = files.map((file, index) => ({ length: file.size, path: [names[index]] }));
    if (options.private)
        info.private = 1;
    const root = { info, "created by": "ToolsBase", "creation date": Math.floor(Date.now() / 1000) };
    if (trackers.length) {
        root.announce = trackers[0];
        root["announce-list"] = trackers.map((tracker) => [tracker]);
    }
    if (options.comment?.trim())
        root.comment = options.comment.trim();
